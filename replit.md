...
- `/users` — Gestão de Usuários (admin only): list, create, edit, toggle active, reset password, delete. Role badges: admin=purple/Shield, profissional=blue/Stethoscope, financeiro=green/Wallet. API: GET/POST /api/users, PUT /api/users/:id, PATCH /api/users/:id/toggle-active, POST /api/users/:id/reset-password, DELETE /api/users/:id (admin-only via requireAdmin middleware).
