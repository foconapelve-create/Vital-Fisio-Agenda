...
### admin
Dashboard, Agenda Geral, Fisio. Pélvica, Agenda Bebês, Confirmações, Aniversariantes, Pacientes, Profissionais, Financeiro, Estoque, Relatórios, Relatório Clínico, Documentos Clínicos, Planner, Usuários

### profissional / fisioterapeuta
Dashboard, Agenda Geral, Fisio. Pélvica, Agenda Bebês, Aniversariantes, Pacientes, Estoque, Relatórios, Relatório Clínico, Documentos Clínicos, Planner. Sem Financeiro/Usuários.

### recepcao (new)
Dashboard, Agenda Geral, Fisio. Pélvica, Agenda Bebês, Confirmações, Aniversariantes, Pacientes. Sem Financeiro/Usuários/Estoque.

### financeiro
Dashboard, Aniversariantes, Financeiro, Estoque, Pacientes, Relatórios.
...
- `/users` — Gestão de Usuários (admin only): list, create, edit, toggle active, reset password, delete. Roles: admin=purple/Shield, profissional=blue/Stethoscope, financeiro=green/Wallet, recepcao=cyan/UserCheck. API: GET/POST /api/users, PUT /api/users/:id, PATCH /api/users/:id/toggle-active, POST /api/users/:id/reset-password, DELETE /api/users/:id.
- `/agenda-pelvica` — Agenda Fisioterapia Pélvica: uses `AgendaEspecializada` component, stores in `especializada_appointments` table with agenda_type='pelvica'. Same time slots as main agenda.
- `/agenda-bebe` — Agenda Bebês: uses `AgendaEspecializada` component with agenda_type='bebe'. Independent from main agenda and pelvica.
- `AgendaEspecializada` component (`src/components/AgendaEspecializada.tsx`): shared weekly grid component for specialized agendas. API prefix: `/api/agenda-esp/`. Props: agendaType, title, clinicName.
- `/reset-password/:token` — public page (no auth). Validates token (30 min expiry), sets new bcrypt password. Token generated by POST /api/auth/forgot-password.
- `/fiscal` — Nota Fiscal (NFSe): full invoice management. Tabs: Notas Fiscais (list with filters/status/search/export), Relatórios (by professional/service), Configurações Fiscais (empresa, endereço, dados fiscais, integração). Emission flow: create → confirm → emit (generates sequential number e.g. "1/000001" + verification code). Cancel with motivo. PDF via browser print popup (printInvoice). Email via mailto:. Integration config: Manual (default, no external API), ABRASF, Nota Carioca, ISSNet, Betha, GINFES, WebISS, outro. Roles: admin + financeiro. Tables: fiscal_settings, invoices, invoice_logs. API routes: fiscal.ts. patients table extended with cpf column.

### DB Tables
- appointments: core appointment table with all 13 statuses
- especializada_appointments: separate table for Fisioterapia Pélvica and Bebês agendas (agenda_type discriminator: 'pelvica' | 'bebe')
